...
if( isset($_GET['doaction']) ){
	...
	delete_ims_orders( );
	else ims_change_status();
}

$order_status = array(
		'trash' 	=> __( 'Trash', $this->domain ),
		'closed' 	=> __( 'Closed', $this->domain ),
		'pending' => __( 'Pending', $this->domain ),
		'shipped' => __( 'Shipped', $this->domain ),
		'publish' => __( 'Published', $this->domain ),
		'cancelled' => __( 'Cancelled', $this->domain ),
		'delete' 	=> __( 'Delete Permanently', $this->domain ),
);
	
$payment_status = array(
		'void' => __( 'Void', $this->domain ),
		'failed' => __( 'Failed', $this->domain ),
		'expired' => __( 'Expired', $this->domain ),
		'denied' => __( 'Denied', $this->domain ),
		'pending' => __( 'Pending', $this->domain ),
		'denided' => __( 'Denided', $this->domain ),
		'refunded' => __( 'Refunded', $this->domain ),
		'reviewing' => __( 'Reviewing', $this->domain ),
		'processed' => __( 'Processed', $this->domain ),
		'completed' => __( 'Completed', $this->domain ),
		'in_progress' => __( 'In Progress', $this->domain ),
);
...
<form class="search-form" action="<?php echo admin_url( 'edit.php?' ) ?>" method="get">
	<p class="search-box">
		...
		<input type="submit" value="<?php esc_attr_e( 'Search Orders', $this->domain ); ?>" class="button" />
...
<form id="posts-filter" action="<?php echo $this->pageurl ?>" method="get">
	<div class="tablenav">
		...
		<div class="alignleft actions">
			<select name="order-action">
				<option value=""><?php esc_attr_e( 'Order Status', $this->domain )?></option>
				<?php foreach( $order_status as $key => $label ){
					if( !$is_trash && $key == 'delete' ) continue;
			...
			<select name="payment-action">
				<option value=""><?php esc_attr_e( 'Payment Status', $this->domain )?></option>
				<?php foreach( $payment_status as $key => $label ){
				 echo '<option value="', esc_attr( $key ) , '">', $label , '</option>';
				}?>
			</select>
			<?php } ?>
			
			<input type="submit" name="doaction" value="<?php esc_attr_e( 'Apply', $this->domain )?>" class="button-secondary action" />
			<select name='m'>
				<option value='0'><?php esc_attr_e( 'Select order date', $this->domain )?></option>
				<?php foreach( ims_order_archive( $status ) as $archive ){
					$date = strtotime( $archive->y . $archive->m ); $val = date( 'Ym', $date );
					echo '<option value="', esc_attr( $val ) , '"', selected( $val, $cdate, false ), '>', date_i18n( 'F Y', $date ) , '</option>';
				}?>
			</select>
			<input type="submit" value="<?php _e( 'Filter', $this->domain )?>" class="button" />
			
			<a href="<?php echo IMSTORE_ADMIN_URL?>/sales-csv.php" class="button"><?php _e( 'Download CSV', $this->domain )?></a>
	...
	<div class="tablenav">
		...
		<div class="tablenav-pages"><?php $page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s' ) . '</span>%s',
			...
			number_format_i18n( min( $page * $this->per_page, $sales->found_posts ) ),
			'<span class="total-type-count">' . number_format_i18n( $sales->found_posts ) . '</span>',
			$page_links
