	 ...
	 function display_rss( ){
		...
		<channel>
			...
			<?php foreach( $ImStore->attachments as $image ){
				...
				$author		= isset( $image->meta['image_meta']['author']) ? $image->meta['image_meta']['author'] 
				: get_the_author_meta( 'display_name', $image->post_author ) ;
			?>
			<item>
				<title><![CDATA[<?php echo $image->post_title ?>]]></title>
				<link><?php echo $ImStore->get_image_url( $image ) ?></link>
				<media:thumbnail url="<?php echo $ImStore->get_image_url( $image, 'thumbnail' )?>"/>
				<media:content type="<?php echo $filetype['type']?>" url="<?php echo $ImStore->get_image_url( $image )  ?>"/>
