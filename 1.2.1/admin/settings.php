...
if ( !empty( $_POST['updateslideshow'] ) ) { 
	...
	foreach( array( '_wpnonce', '_wp_http_referer', 'updateslideshow' ) as $key )
		unset( $_POST[$key] );
		
	foreach( array( 'autoStart' ) as $box )
		if( empty( $_POST[$box] ) ) $_POST[$box] = '';

	update_option( 'ims_front_options', wp_parse_args( $_POST, $this->opts ) );
	wp_redirect( $pagenowurl . '&ms=4#slideshow-settings' );	
}
...
<div class="wrap imstore">
	...
	<!-- Slideshow Settings -->
 	...
 	<div id="slideshow-settings" class="ims-box">
		...
		<?php wp_nonce_field( 'ims_slideshow_settings' )?>
		<table class="ims-table"> 
			<tbody>
				<tr>
				...
				<tr>
					...
					<td colspan="3"><input type="submit" name="updateslideshow" class="button-primary" value="<?php _e( 'Save Changes', ImStore::domain )?>"/></td>
					</tr>
			</tbody>
		</table>
		</form>
